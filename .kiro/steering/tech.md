...
- **AWS CDK v2** (`aws-cdk-lib ^2.231.0`) with TypeScript
- **`@orcabus/platform-cdk-constructs`** — internal shared constructs library (pinned to `0.0.98`)
- **`@aws-cdk/aws-lambda-python-alpha`** — Python Lambda bundling via `PythonUvFunction` (uses `uv`)
- **cdk-nag** (`^2.37.55`) — CDK security/compliance rule checks in tests
...
- **Amazon EventBridge** — event bus `OrcaBusMain`, source `orcabus.arribawgtsrna`
- **AWS SSM Parameter Store** — configuration under `/orcabus/workflows/arriba-wgts-rna/`
- **AWS Schemas Registry** — JSON schema validation for draft events

## Package Manager

**pnpm** (v10.17.1). Always use `pnpm`, never `npm` or `yarn`.
...
- **Jest** (`^29.7.0`) with `ts-jest` for CDK infrastructure tests
